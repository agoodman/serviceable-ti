( function() {
		...
		base.refreshBearerToken = function(aOptions) {
			...
			if( base.tokenExpiresAt && moment() < moment(base.tokenExpiresAt) ) {
				// found current token; no need to refresh
				tSuccess();
				return;
			}
			tClient.onload = function() {
				...
				if (tRsp.token_type == "bearer") {
					...
					base.tokenExpiresAt = moment().add('seconds',tRsp.expires_in).toISOString();
					if( tRsp.refresh_token ) {
						base.refreshToken = tRsp.refresh_token;
					}else{
			...
			if( base.tokenExpiresAt && base.refreshToken && moment() > moment(base.tokenExpiresAt) ) {
		...
		base.retrieveLocalObject = function(aObj, aId, aOptions) {
			Ti.API.info("retrieving " + aObj + " local_id=" + aId);
			var tCollection = Alloy.Collections.instance(aObj);
			tCollection.fetch({
				query : 'select * from ' + aObj + ' where local_id = ' + aId + ' limit 1',
				success : function() {
					var tFirst = tCollection.at(0);
					aOptions.success(tFirst);
				}
			});
		};
		...
		base.loadNext = function(aObj, aPage, aSince) {
			var tClient = base.clientFactory();
			if (tClient == null) {
			...
			tClient.onload = function() {
				...
				if (tObjs && tObjs.length != 0) {
					base.loadNext(aObj, aPage + 1, aSince);
				}
			};
			tClient.onerror = function(e) {
				Ti.API.error("unable to sync " + tEndpoint);
			};
			tClient.open("GET", base.host + base.basePath + base.urlHelper(tEndpoint, aSince, aPage));
			if (base.bearerToken) {
				tClient.setRequestHeader("Authorization", "Bearer " + base.bearerToken);
			}
			tClient.send();
			Ti.API.info("loading " + tEndpoint + " since " + aSince);
		};
			...
			tObjs.fetch({
				...
				success : function() {
					...
					_.each(tObjs.select(function(e) {
						...
						tClient.open('POST', base.host + base.basePath + tPath + "&" + tParams, false);
						if (base.bearerToken) {
							tClient.setRequestHeader("Authorization", "Bearer " + base.bearerToken);
						}
						if (tBeforeCreate) {
					...
					_.each(tObjs.select(function(e) {
						...
						tClient.open('PUT', base.host + base.basePath + tPath + "&" + tParams);
						Ti.API.info("PUT " + tPath + "&" + tParams);
						if (base.bearerToken) {
							tClient.setRequestHeader("Authorization", "Bearer " + base.bearerToken);
						}
						if (tBeforeUpdate) {
					...
					_.each(tObjs.select(function(e) {
						...
						tClient.open('DELETE', base.host + base.basePath + tPath, false);
						if (base.bearerToken) {
							tClient.setRequestHeader("Authorization", "Bearer " + base.bearerToken);
						}
						tClient.send();
		...
		function deepQueryString(aObj, aKeyPath, aWrap) {
			...
			Ti.API.info("results: "+JSON.stringify(tResults));
			return tResults;
		}
